...
@pytest.mark.parametrize(
    ("state",),
    (
        (State(frozenset((at(left, b2), at(right, b2), control(left)))),),
        (State(frozenset((at(left, b1), at(right, b1), control(left)))),),
        (State(frozenset((at(left, b1), at(right, b1), border(right, (b1, b2)), control(right)))),),
        (
            State(
                frozenset(
                    (at(left, b2), at(right, b1), border(right, (b1, b2)), revealed(right, (b1, b2)), control(right))
                )
            ),
        ),
    ),
...
@pytest.mark.parametrize(
    ...
    (
        (State(frozenset((at(left, b2), at(right, b2), control(left)))),),
        (State(frozenset((at(left, b4), at(right, b3), control(right)))),),
    ),
)
def test_get_goals(
    reference_interpreter: ClingoInterpreter, interpreter: DarkSplitCorridor34Interpreter, state: State
...
@pytest.mark.parametrize(
    ...
    (
        (State(frozenset((at(left, b2), at(right, b2), control(left)))),),
        (State(frozenset((at(left, a4), at(right, a3), control(right)))),),
    ),
)
